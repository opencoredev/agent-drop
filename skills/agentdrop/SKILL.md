...
There is a stateless MCP server at `https://abundant-poodle-82.convex.site/mcp` (Streamable HTTP, revision
...
```bash
claude mcp add --transport http agentdrop https://abundant-poodle-82.convex.site/mcp     # Claude Code
codex mcp add agentdrop --url https://abundant-poodle-82.convex.site/mcp                  # Codex
gemini mcp add --transport http agentdrop https://abundant-poodle-82.convex.site/mcp      # Gemini CLI
code --add-mcp '{"name":"agentdrop","type":"http","url":"https://abundant-poodle-82.convex.site/mcp"}'  # VS Code
```
...
```bash
curl -X POST https://abundant-poodle-82.convex.site/api/v1/sites/<slug>/images \
  -H "Authorization: Bearer <editToken>" \
  -H "Content-Type: image/png" \
  --data-binary @diagram.png
# → { "url": "https://abundant-poodle-82.convex.site/api/v1/assets/<id>" }
```
